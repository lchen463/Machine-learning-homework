...
def plot_feature_importances(clf, feature_names):
    ...
    plt.savefig('feature.png', fmt='png',dpi=300)
...
def plot_cv(variable,score):
    ...
    plt.savefig('CV.png', fmt='png',dpi=300)
...
def plot_learning_curve(train_sizes,train_scores_mean,train_scores_std,test_scores_mean,test_scores_std,title,feature):
 ...
 #   if ylim is not None:
  #      plt.ylim(*ylim)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(str(feature)+title+'.png',format='png',dpi=300)
    return plt
...
train_size=[0.4,0.5,0.6,0.7,0.8,0.9]    
...
for lc in train_size:    
    train_scores=[]
    test_scores=[]
    
    for seed in seeds:
       
#for lc in [0.01, 0.001, 0.0001]:   ###learning-rate, best 0.01
        X_train, X_test, Y_train, Y_test = train_test_split(X, y, test_size=1-lc,random_state=seed)
 ...
 #       plt.figure(0)
  #      print(history.history.keys())
        ...
        # plt.plot(history.history['loss'],label='lr: '+str(lc)+', train')
        # plt.plot(history.history['val_loss'],label='lr: '+str(lc)+', validation')
        # plt.title('model loss')
        # plt.ylabel('loss')
        # plt.xlabel('epoch')
        # plt.legend(loc=1)
        # plt.savefig(str(lc)+'loss.png',dpi=300)
        ...
        # plt.plot(history.history['accuracy'],label='lr: '+str(lc)+', train')
        # plt.plot(history.history['val_accuracy'],label='lr: '+str(lc)+', validation')
        
        # plt.title('model accuracy')
        # plt.ylabel('Accuracy')
        # plt.xlabel('epoch')
        # plt.legend(loc=4)
        # plt.savefig(str(lc)+'_accuracy.png',dpi=300)
...
plot_learning_curve(train_sizes,train_scores_mean,train_scores_std,test_scores_mean,test_scores_std,'Learning Curve of' +task,task)
