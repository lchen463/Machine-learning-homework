...
def plot_cv(variable,score,x):
    ...
    plt.savefig(x+'CV.pdf', fmt='pdf',dpi=300)
...
for val in max_depth:
   ...
   for l in leaf:
        ...
        score = cross_val_score(GradientBoostingClassifier(n_estimators=val,learning_rate=1.0,max_depth=l,random_state= 0), X, y, cv= 5, scoring="accuracy")
        score_mean=score.mean()
        score_cv.append(score_mean)
            
        print(f'Average score({val}): {"{:.3f}".format(score.mean())}')
            
   plt.plot(leaf,score_cv,'o-',label='N_estimators: '+str(val))
   plt.legend()
...
for size in train_size:    
   ...
   for seed in seeds:
        ...
        clf1 =GradientBoostingClassifier(n_estimators=40,learning_rate=1.0,max_depth=3)
